...
// Require SSL only for remote (hosted) databases such as the Supabase pooler.
// Local development databases (localhost/127.0.0.1) usually run without TLS.
const isLocal = ["localhost", "127.0.0.1", "::1"].includes(migrationUrl.hostname);
if (!isLocal) {
