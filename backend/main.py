...
def get_capabilities():
    ...
    return ProviderCapabilitiesResponse(
        ...
        image_models=[
            ProviderModelOption(id="default", label="Best available", resolutions=["640px", "1k", "2k", "4k"], durations=[], supports_audio=False),
            ProviderModelOption(id="flux-schnell", label="Flux Schnell — fast image draft", resolutions=["640px", "1k", "2k"], durations=[], supports_audio=False),
            ProviderModelOption(id="flux-2-klein", label="Flux 2 Klein — concise image work", resolutions=["640px", "1k", "2k"], durations=[], supports_audio=False),
            ProviderModelOption(id="z-image-turbo", label="Z-Image Turbo — fast image draft", resolutions=["640px", "1k", "2k"], durations=[], supports_audio=False),
        ],
        image_style_tools=["general", "ai-photo-generator", "ai-illustration-generator", "ai-logo-generator", "movie-poster-generator", "thumbnail-maker"],
...
async def generate_videos(req: GenerateRequest):
    ...
    for scene in req.scenes:
        job_id = await magic_hour.submit_text_to_video(prompt=scene.visual_prompt, duration_seconds=scene.duration_seconds, aspect_ratio=req.options.aspect_ratio, model=req.options.model, resolution=req.options.resolution, audio=req.options.audio, name=f"Werkbank scene {scene.index + 1}")
        jobs.append(remember_job(job_id, kind="video", workflow="text-to-video", scene_index=scene.index))
    return GenerateResponse(jobs=jobs)


@app.post("/api/images/generate", response_model=SceneJob)
async def generate_image(req: ImageGenerationRequest):
    job_id = await magic_hour.submit_text_to_image(prompt=req.prompt, aspect_ratio=req.aspect_ratio, model=req.model, resolution=req.resolution, style_tool=req.style_tool, name=req.name)
...
async def generate_video_from_image(req: ImageToVideoRequest):
    if req.asset_id:
        ...
        if asset is None:
            raise HTTPException(status_code=404, detail="The image reference expired. Upload the image again before generating the video.")
        image_file_path = asset.provider_file_path
    else:
        image_file_path = await magic_hour.prepare_generated_image(req.image_project_id or "")
    job_id = await magic_hour.submit_image_to_video(image_file_path=image_file_path, prompt=req.prompt, duration_seconds=req.duration_seconds, model=req.options.model, resolution=req.options.resolution, audio=req.options.audio, name=req.name)
...
async def check_status(job_id: str):
    ...
    if metadata is None:
        raise HTTPException(status_code=404, detail="This project is not available in the current server session. Create a new job or add persistent storage.")
    result = await magic_hour.poll_project(project_id=job_id, kind=metadata.kind)
    return StatusResponse(job_id=job_id, kind=metadata.kind, workflow=metadata.workflow, status=result["status"], video_url=result["video_url"], image_urls=result["image_urls"], error=result["error"])
